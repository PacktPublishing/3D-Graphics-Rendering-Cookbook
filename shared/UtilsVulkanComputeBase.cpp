#include "shared/UtilsVulkanComputeBase.h"
...
bool ComputeBase::execute(uint32_t xsize, uint32_t ysize, uint32_t zsize)
{
	return executeComputeShader(vkDev, pipeline, pipelineLayout, descriptorSet, xsize, ysize, zsize);
}

bool ComputeBase::uploadInput(uint32_t offset, void* inData, uint32_t byteCount)
{
	uploadBufferData(vkDev, inBufferMemory, offset, inData, byteCount);
	return true;
}

bool ComputeBase::downloadOutput(uint32_t offset, void* outData, uint32_t byteCount)
{
