...
#include "Chapter9/GLSkyboxRenderer.h"
...
int main(void)
{
	...
	// scene
	GLShader shaderVert("data/shaders/chapter09/GL01_scene_IBL.vert");
	GLShader shaderFrag("data/shaders/chapter09/GL01_scene_IBL.frag");
	...
	// OIT
	GLShader shaderFragOIT("data/shaders/chapter09/GL03_mesh_oit.frag");
	GLProgram programOIT(shaderVert, shaderFragOIT);
	GLShader shdCombineOIT("data/shaders/chapter09/GL03_OIT.frag");
	GLProgram progCombineOIT(shdFullScreenQuadVert, shdCombineOIT);
	// GPU culling
	GLShader shaderCulling("data/shaders/chapter09/GL02_FrustumCulling.comp");
	...
	// shadows
	GLShader shdShadowVert("data/shaders/chapter09/GL05_shadow.vert");
	GLShader shdShadowFrag("data/shaders/chapter09/GL05_shadow.frag");
	...
	while (!glfwWindowShouldClose(app.getWindow()))
	{
		...
		PerFrameData perFrameData = {
			...
			.enableCulling = g_EnableGPUCulling ? 1u : 0u
		};
		...
		// cull
		{
			*numVisibleMeshesPtr = 0;
			...
			glDispatchCompute((GLuint)meshesOpaque.drawCommands_.size(), 1, 1);

			glBindBufferBase(GL_SHADER_STORAGE_BUFFER, kBufferIndex_DrawCommands, meshesTransparent.getHandle());
			glDispatchCompute((GLuint)meshesTransparent.drawCommands_.size(), 1, 1);
