...
int main(void)
{
	...
	while (!glfwWindowShouldClose(app.getWindow()))
	{
		...
		PerFrameData perFrameData = { .view = view, .proj = proj, .light = mat4(0.0f), .cameraPos = glm::vec4(camera.getPosition(), 1.0f), .enableCulling = g_EnableGPUCulling ? 1u : 0u };
		...
		*numVisibleMeshesPtr = 0;
		...
		glDispatchCompute((GLuint)sceneData.shapes_.size(), 1, 1);
