...
struct AtomicRenderer: public Renderer
{
	AtomicRenderer(VulkanRenderContext& ctx, VulkanBuffer sizeBuffer): Renderer(ctx)
	{
		...
		initPipeline({ "data/shaders/chapter09/VK01_AtomicTest.vert", "data/shaders/chapter09/VK01_AtomicTest.frag" }, pInfo);
	}
...
struct AnimRenderer : public Renderer
{
		...
		pointBuffers_(pointBuffers)
	{
		...
		initPipeline({ "data/shaders/chapter09/VK01_AtomicVisualize.vert", "data/shaders/chapter09/VK01_AtomicVisualize.frag" }, PipelineInfo{ .topology = VK_PRIMITIVE_TOPOLOGY_POINT_LIST });
	}
